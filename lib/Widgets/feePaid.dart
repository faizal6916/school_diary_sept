...
class FeePaid extends StatefulWidget {
  ...
  const FeePaid({Key? key,this.voucherNo,this.totalAmount,this.transactionDate,this.detailList}) : super(key: key);
...
class _FeePaidState extends State<FeePaid> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            width: 1.sw,
            height: 45 * double.parse(widget.detailList!.length.toString()) + 60,
            color: Colors.white,
            child: Padding(
                    ...
                    child: DottedBorder(
                        ...
                        child: Table(
                          ...
                          children: [
                            ...
                            //   ]
                            // )).toList()
                            ...widget.detailList!.asMap().map((index, element) => MapEntry(index, TableRow(
                              decoration: BoxDecoration(
                                color: (index % 2 == 0)? Colors.transparent:ColorUtil.paidBor.withOpacity(0.2)
                              ),
                                children: [
                              Padding(
                                padding: const EdgeInsets.all(10.0),
                                child: Text('${element['transaction_desc']}',style: feeListStyle(),),
                              ),
                                  Padding(
                                    padding: const EdgeInsets.all(10.0),
                                    child: Text(':',style: feeListStyle(),),
                                  ),
                                  Padding(
                                    padding: const EdgeInsets.all(10.0),
                                    child: Text('AED ${element['transaction_amount']}',style: feeListStyle(),),
                                  )
                            ]))).values.toList()
                          ],
                        )),
                    ...
                    children: [
                      TableRow(
                        children: [
                          Padding(
                            padding: const EdgeInsets.all(10.0),
                            child: Text('Grand Total',style: feeTotalStyle(),),
                          ),

                          Padding(
                            padding: const EdgeInsets.all(10.0),
                            child: Text(':',style: feeTotalStyle(),),
                          ),
                          Padding(
                            padding: const EdgeInsets.all(10.0),
                            child: Text('AED ${widget.totalAmount!}',style: feeTotalStyle(),),
                          )
                        ]
                      )
                    ],
  ...
  Widget rightPart({String? rcptNo,String? totalPaid,String? traDate}) => Container(
    width: 1.sw*2/3,
    height: 140,
    decoration: BoxDecoration(
      color: Colors.white,
  ...
  Widget leftPart({required String date}) => Container(
    ...
    child: Text(
     '${DateFormat('MMM').format(DateTime.parse('${date.split('-').last}-${date.split('-')[1]}-${date.split('-').first}')).toString().toUpperCase()}' ,
      style: TextStyle(
          color: Colors.white, fontSize: 18.sp, fontFamily: 'Axiforma',fontWeight: FontWeight.w700),
    ),
    decoration: BoxDecoration(
      color: Color(0xff26de81),
      borderRadius: BorderRadius.only(
